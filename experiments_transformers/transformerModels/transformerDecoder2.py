...
class TransformerDecoderModel2(pl.LightningModule):
    ...
    def __init__(self,input_size,output_size, n_features, d_model=256,nhead=8, num_layers=3, dropout=0.1):
        ...
        decoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead,dim_feedforward=d_model*4, dropout=dropout, activation='relu',batch_first=True)
    ...
    def forward(self, src):
        ...
        if self.src_mask is None or self.src_mask.size(1) != src.shape[1]:
            self.src_mask = generate_square_subsequent_mask(src.shape[1]).to(src.device)
