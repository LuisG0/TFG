#From: https://github.com/Flawless1202/Non-AR-Spatial-Temporal-Transformer/blob/5bf9b984272e21a63b464caa0c26d8097cb0b5a0/nast/models/utils/spatial_temporal_tensor_attention.py#L171
...
from transformerModels.utils import PositionalEncoding, generate_square_subsequent_mask
...
if __name__ == '__main__':
    ...
    inp = torch.rand(16, 15, 9, 1)
    shape = inp.shape
    inp = torch.rand(16, 15, 9, 64)
    key_padding_mask = inp.new_ones(shape).bool()
    attn_mask = None
    if attn_mask is None or attn_mask.size(0) != inp.shape[1]:
        attn_mask = _get_subsequent_mask(inp).to(inp.device)
    print("type", attn_mask)
    out = stta(inp, inp, inp,key_padding_mask,attn_mask=attn_mask)
    print(out[0].size())